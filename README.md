...
| `CLASS_ACCESS_CODE` | (선택) 설정 시 `/api/auth/students`에 접근 코드 필요. 미설정 시 기존과 동일 |

프론트 개발/빌드 시 `CLASS_ACCESS_CODE`를 쓰면 **`frontend/.env`에 `VITE_CLASS_ACCESS_CODE`** 를 같은 값으로 넣어야 학생 로그인 목록이 동작합니다.
...
- 로그인 성공 시 `localStorage`에는 **비밀번호가 아닌 서명 토큰**만 저장됩니다 (유효기간 7일).
- 예전에 저장된 `admin:비밀번호` 토큰은 재로그인 전까지 동작합니다.

### 데이터 백업 (`/api/export/all`)

- 운동·인바디·목표·루틴·운동 라이브러리·**수면·식단** 포함.
- 사진은 기본 **메타만** (용량 절약). `?withPhotos=1` 또는 리포트 화면의 “사진 포함” 체크.
